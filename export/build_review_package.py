...
PORT_CALL_DESC = {
    ...
    "agency": "Call-level agency: a SINGLE pick for the whole call. NOT THE COLUMN FOR REVENUE-BY-AGENT REPORTING -- use PORT_CALL_LEG::agency instead. A split call can be worked by two agencies; 91 calls carry more than one across their legs, and using this column instead of the leg one mis-attributes $939,000 of fee to the wrong agent (not lost from the total -- credited to whoever won the pick). See docs/BUSINESS_RULES.md section 6.",
...
PORT_CALL_LEG_DESC = {
    ...
    "agency": "THE COLUMN TO USE FOR REVENUE-BY-AGENT REPORTING. The inbound agency of this leg specifically -- the agency that brought the vessel in owns the leg (docs/BUSINESS_RULES.md section 6). Prefer this over PORT_CALL::agency, which is a single call-level pick.",
    "agency_source": "Where the leg's agency was determined from: 'inbound' / 'leg' / 'call' / 'none'.",
    "agent_changed_in_leg": "1 if the source data showed more than one distinct agency inside this leg. MATERIAL: 3,233 chargeable legs (8.03%) carrying $29,495,250 (10.82% of the billable total) are flagged here. The fee still goes wholly to the inbound agency by rule, so a by-agent revenue report is not a clean division of the book -- about $1 in $9 is attributed to an agent where another agency was also involved. Ordinary on a chartered voyage: owner's agent inbound, charterer's agent on the outbound load. See docs/BUSINESS_RULES.md section 6.",
    "cargo_group": "From FGIS ('Grain'), else the zone dictionary's typical cargo group for the first berth.",
    "cargo_subgroup": "Splits cargo_group where the evidence allows. 'Certified grain' wherever an FGIS certificate matched (10,545 legs); otherwise whatever the berth has DECLARED its uncertified cargo to be ('Grain by-product', 254 legs, MGMT only); blank where nothing is known. Deliberately NOT inferred from a missing certificate -- absence proves by-product at MGMT but proves only a failed match at an elevator running 99.6% coverage, so inferring it globally would invent by-product cargo at nine grain elevators. See docs/BUSINESS_RULES.md section 8.",
...
PORT_CALL_EVENT_DESC = {
    ...
    "cargo_subgroup": "Carried down from the leg. See PORT_CALL_LEG::cargo_subgroup.",
